  ...
  if (isValidating) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-400 text-xs uppercase tracking-widest font-bold">Verifying Security Token...</p>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="space-y-6 text-center animate-in fade-in zoom-in-95 duration-500">
        ...
        <h2 className="text-xl font-black text-white uppercase tracking-widest">Password Updated</h2>
        <p className="text-xs text-slate-400 mt-2">
          Your credentials have been securely updated. <br/>Redirecting you to the login portal...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center mb-8">
        <h2 className="text-xl font-black text-white uppercase tracking-widest">Set New Password</h2>
        <p className="text-xs text-slate-400 mt-2">Resetting security credentials for <span className="text-white font-bold">{email}</span></p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="pt-4">
          ...
          >
            {isLoading ? 'Processing Security Update...' : 'Update Password'}
          </button>
        </div>
      </form>
    </div>
      ...
      <div className="max-w-md w-full space-y-8 glass-card p-10 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Suspense fallback={
            <div className="flex flex-col items-center justify-center space-y-4">
              <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-brand-accent"></div>
              <p className="text-slate-400 text-xs uppercase tracking-widest font-bold">Initialising Security Protocol...</p>
            </div>
          }>
            <ResetPasswordForm />
          </Suspense>
