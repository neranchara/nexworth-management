...
export const HealthCard = ({ fullMode = false }: { fullMode?: boolean }) => {
  ...
  const statusColor = data?.status === 'red' ? 'text-rose-500' : data?.status === 'yellow' ? 'text-amber-500' : 'text-emerald-500';
  const statusBg = data?.status === 'red' ? 'bg-rose-500/10' : data?.status === 'yellow' ? 'bg-amber-500/10' : 'bg-emerald-500/10';

  return (
    <div className={`bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 flex flex-col gap-6 ${fullMode ? 'h-full' : ''}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white">System Health Monitor</h2>
            <p className="text-[10px] text-slate-500 font-medium uppercase tracking-[0.2em]">Real-time Status Suite</p>
          </div>
        </div>
      ...
      {fullMode && (
        <div className="flex-1 mt-4 p-6 bg-slate-950/50 border border-slate-800 rounded-2xl flex flex-col gap-4">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Live Resource Telemetry</h3>
          <div className="flex-1 border-b border-dashed border-slate-800 relative">
             ...
             <div className="absolute inset-0 flex items-end gap-1 px-2 pb-2">
                {Array.from({ length: 40 }).map((_, i) => (
                  <div 
                    key={i} 
                    className="flex-1 bg-indigo-500/20 rounded-t-sm animate-pulse" 
...
function MetricItem({ icon, label, value, status }: { icon: React.ReactNode, label: string, value: string, status: 'success' | 'warning' | 'danger' }) {
  const colorClass = status === 'success' ? 'text-emerald-400' : status === 'warning' ? 'text-amber-400' : 'text-rose-400';
  return (
    <div className="p-4 bg-slate-800/30 border border-slate-700/50 rounded-xl flex flex-col gap-2">
      <div className="flex items-center justify-between text-slate-500">
        {icon}
        <div className={`w-1.5 h-1.5 rounded-full ${status === 'success' ? 'bg-emerald-500' : status === 'warning' ? 'bg-amber-500' : 'bg-rose-500'}`} />
      </div>
