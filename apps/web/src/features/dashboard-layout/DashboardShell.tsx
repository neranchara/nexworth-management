    ...
    <div className="h-screen w-screen flex overflow-hidden bg-midnight text-slate-100 selection:bg-emerald/30">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative bg-gradient-to-tr from-midnight via-midnight to-navy/30">
        ...
        {isImpersonating && impersonationData && (
          <div className="bg-amber-500/10 border-b border-amber-500/20 text-amber-400 px-6 py-2.5 flex items-center justify-between text-xs font-semibold shrink-0 shadow-md">
            <div className="flex items-center gap-2">
              ...
              <span>
                VIEW-AS MODE ACTIVE: Impersonating <strong className="text-white underline">{impersonationData.email}</strong> (Read-Only Security Enabled)
              </span>
            </div>
            <button
              onClick={stopViewAs}
              className="px-3 py-1 bg-amber-500 hover:bg-amber-600 active:scale-95 text-midnight rounded font-black uppercase text-[10px] tracking-wider transition-all shadow shadow-amber-500/25"
            >
              Stop View-As
            </button>
          </div>
