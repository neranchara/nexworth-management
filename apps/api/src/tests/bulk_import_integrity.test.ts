...
describe('Bulk Import Data Integrity & Atomicity', () => {
  ...
  beforeEach(() => {
    ...
    (prisma.asset.aggregate as any).mockResolvedValue({ _sum: { amount: 0 } });
    (prisma.liability.aggregate as any).mockResolvedValue({ _sum: { amount: 0 } });
    (prisma.transaction.aggregate as any).mockResolvedValue({ _sum: { amount: 0 } });
    (prisma.asset.findUnique as any).mockResolvedValue({ id: 'ast-1', amount: 0 });
    (prisma.liability.findUnique as any).mockResolvedValue({ id: 'lia-1', amount: 0 });
  });
