...
When the user orders to "create a new version", you MUST follow these steps:
1. **Request Version Number**: Ask the developer (user) for the specific version number (e.g., `2.1.0`).
2. **Create Branch**: Create a new branch using that version number based on the current `main`.
   - **Command**: `git checkout main`, `git pull`, `git checkout -b nexworth-v-<version>`
3. **Switch & Develop**: Switch to this branch immediately for all development work.

## 2. Code Management & Versioning (Git Commit/Merge)
