import { PrismaClient } from '@prisma/client';
const prisma = new PrismaClient();

async function main() {
  console.log('--- Starting Transaction & Loan Migration ---');
  
  // 1. Fetch all Assets and Liabilities to build a lookup map
  const assets = await prisma.asset.findMany();
  const liabilities = await prisma.liability.findMany();
  
  const assetMap = new Map(assets.map(a => [a.accountId, a.id]));
  const liabilityMap = new Map(liabilities.map(l => [l.accountId, l.id]));
  
  // 2. Migrate Transactions
  const transactions = await prisma.transaction.findMany({
    where: { assetId: null, liabilityId: null }
  });
  console.log(`Found ${transactions.length} transactions to migrate.`);
  
  for (const tx of transactions) {
    const assetId = assetMap.get(tx.accountId);
    const liabilityId = liabilityMap.get(tx.accountId);
    
    if (assetId || liabilityId) {
      await prisma.transaction.update({
        where: { id: tx.id },
        data: { assetId, liabilityId }
      });
    } else {
      console.warn(`No Asset or Liability found for Account ID: ${tx.accountId} (Transaction: ${tx.id})`);
    }
  }
  
  // 3. Migrate Loans
  const loans = await prisma.loan.findMany({
    where: { assetId: null, liabilityId: null }
  });
  console.log(`Found ${loans.length} loans to migrate.`);
  
  for (const loan of loans) {
    const assetId = assetMap.get(loan.accountId);
    const liabilityId = liabilityMap.get(loan.accountId);
    
    if (assetId || liabilityId) {
      await prisma.loan.update({
        where: { id: loan.id },
        data: { assetId, liabilityId }
      });
    } else {
      console.warn(`No Asset or Liability found for Account ID: ${loan.accountId} (Loan: ${loan.id})`);
    }
  }
  
  console.log('--- Migration Completed ---');
}

main().catch(console.error).finally(() => prisma.$disconnect());
